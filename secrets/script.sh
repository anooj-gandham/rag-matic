...
# Construct IMAGE_REPO
IMAGE_REPO="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com"

# Verify IMAGE_REPO is set
...
docker run -d --name django-app -p 8000:8000 --env-file /home/ubuntu/.env $IMAGE_REPO/django-app:latest

echo "Starting Celery worker (embeddings)..."
docker run -d --name celery-embeddings --env-file /home/ubuntu/.env $IMAGE_REPO/celery-embeddings:latest

echo "Starting Celery worker (queries)..."
docker run -d --name celery-queries --env-file /home/ubuntu/.env $IMAGE_REPO/celery-queries:latest
